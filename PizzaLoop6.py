...
def obter_pedidos_da_tabela():
    ...
    try:
        for item_id in tabela.get_children():
            ...
            if not valores:
                continue
            numero = int(str(valores[0]).replace("#", ""))
            pedido = next((p for p in pedidos_dados if p["numero"] == numero), None)
            if pedido:
...
def obter_pedidos_filtrados_relatorio():
    pedidos_filtrados = obter_pedidos_da_tabela() or pedidos_dados
    if filtro_inicio_data:
        pedidos_filtrados = [
            p for p in pedidos_filtrados if p["data_hora_obj"].date() >= filtro_inicio_data
        ]
    if filtro_fim_data:
        pedidos_filtrados = [
            p for p in pedidos_filtrados if p["data_hora_obj"].date() <= filtro_fim_data
        ]
...
def desenhar_grafico_status(status_counts):
    ...
    if total == 0:
        ...
        canvas_grafico.create_text(center_x, center_y, text="Sem pedidos", fill=COR_MUTED, font=("Arial", 11, "bold"))
    else:
    ...
    for nome, valor, cor in dados:
        ...
        canvas_grafico.create_rectangle(legend_x, legend_y, legend_x + 18, legend_y + 18, fill=cor, outline="")
        canvas_grafico.create_text(
            legend_x + 30,
            legend_y + 9,
            text=f"{nome}: {percentual}%",
            anchor="w",
            fill=COR_TEXTO,
            font=("Arial", 10),
        )
        legend_y += 28
    canvas_grafico.create_text(470, 260, text="Base: distribuição de pedidos por status", fill=COR_MUTED, font=("Arial", 9), anchor="w")
...
def desenhar_grafico_receita(faturamento_bruto, faturamento_liquido, valor_cancelado, pagamentos):
    ...
    for nome, valor, cor in dados:
        ...
        largura = int((pct / 100) * largura_total)
        canvas_grafico_receita.create_rectangle(x, y, x + largura, y + altura, fill=cor, outline="")
        canvas_grafico_receita.create_text(
            x + 10,
            y + altura // 2,
            text=f"{nome}: {moeda_br(valor)} ({pct:.0f}%)",
            anchor="w",
            fill="white" if pct > 15 else COR_TEXTO,
            font=("Arial", 9, "bold"),
        )
        y += 30
    canvas_grafico_receita.create_text(
        20,
        y + 20,
        text="Receita proporcional por categoria.",
        anchor="w",
        fill=COR_MUTED,
        font=("Arial", 9),
    )
...
def atualizar_relatorio():
    ...
    if lbl_faturamento_bruto is not None:
        lbl_faturamento_bruto.config(text=moeda_br(faturamento_bruto))
    if lbl_faturamento_liquido is not None:
        lbl_faturamento_liquido.config(text=moeda_br(faturamento_liquido))
    if lbl_total_cancelado is not None:
        lbl_total_cancelado.config(text=moeda_br(sum(p["total"] for p in cancelados)))
    if lbl_qtd_pedidos is not None:
        lbl_qtd_pedidos.config(text=str(qtd))
    if lbl_qtd_cancelados is not None:
        lbl_qtd_cancelados.config(text=str(qtd_cancelados))
    if lbl_ticket_medio is not None:
        lbl_ticket_medio.config(text=moeda_br(ticket))
...
def exportar_relatorio_word():
    """Exporta os pedidos filtrados para um arquivo Word com todos os status."""
    pedidos = obter_pedidos_filtrados_relatorio()
    if not pedidos:
        messagebox.showinfo("Exportar", "Nenhum pedido para exportar.")
        return

    default_name = datetime.now().strftime("relatorio_pedidos_%Y%m%d_%H%M%S.doc")
    caminho = filedialog.asksaveasfilename(defaultextension=".doc", filetypes=[("Word Document", "*.doc")], initialfile=default_name)
    if not caminho:
        return
    ...
    try:
        html = [
            ...
            f"<p>Data do relatório: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}</p>",
            "<table border=\"1\" cellpadding=\"6\" cellspacing=\"0\" style=\"border-collapse: collapse; width: 100%; font-family: Arial, sans-serif;\">",
            "<thead>",
            "<tr>",
            "<th>Pedido</th><th>Data/Hora</th><th>Cliente</th><th>Item</th><th>Total</th><th>Pagamento</th><th>Status</th>",
            "</tr>",
            "</thead>",
            "<tbody>",
        ]
        for p in pedidos:
            html.append("<tr>")
            html.append(f"<td>{p['numero']}</td>")
            html.append(f"<td>{p['data_hora_str']}</td>")
            html.append(f"<td>{p['cliente']}</td>")
            html.append(f"<td>{p['item']}</td>")
            html.append(f"<td>R$ {p['total']:.2f}</td>")
            html.append(f"<td>{p['pagamento']}</td>")
            html.append(f"<td>{p['status']}</td>")
            html.append("</tr>")
        html.extend([
            "</tbody>",
            "</table>",
        ...
        messagebox.showinfo("Exportar", f"Documento do Word exportado em:\n{caminho}")
    except Exception as e:
        messagebox.showerror("Exportar", f"Erro ao exportar Word:\n{e}")
...
def fechar_relatorio():
    ...
    global lbl_faturamento_bruto, lbl_faturamento_liquido, lbl_total_cancelado, lbl_qtd_pedidos, lbl_qtd_cancelados, lbl_ticket_medio
...
def abrir_relatorio_vendas():
    ...
    Button(
        filtro, text="Exportar Relatório", command=exportar_relatorio_word, bg="#1d4ed8", fg="white", bd=0, padx=10
    ).grid(row=1, column=4, padx=4)

    kpis = Frame(corpo, bg=COR_BG)
    kpis.pack(fill="x", pady=10)

    resumo_card = Frame(kpis, bg=COR_CARD, bd=1, relief="solid")
    resumo_card.pack(fill="x", pady=4)
    Label(resumo_card, text="Resumo financeiro", bg=COR_CARD, fg=COR_TEXTO, font=("Arial", 11, "bold")).pack(anchor="w", padx=10, pady=(10, 0))

    preco_frame = Frame(resumo_card, bg=COR_CARD)
    preco_frame.pack(fill="x", padx=10, pady=10)
    preco_frame.grid_columnconfigure(0, weight=1)
    preco_frame.grid_columnconfigure(1, weight=1)

    Label(preco_frame, text="Faturamento bruto:", bg=COR_CARD, fg=COR_TEXTO).grid(row=0, column=0, sticky="w", padx=4, pady=3)
    lbl_faturamento_bruto = Label(preco_frame, text="R$ 0.00", bg=COR_CARD, fg=COR_TEXTO, font=("Arial", 11, "bold"))
    lbl_faturamento_bruto.grid(row=0, column=1, sticky="e", padx=4, pady=3)

    Label(preco_frame, text="Faturamento líquido:", bg=COR_CARD, fg=COR_TEXTO).grid(row=1, column=0, sticky="w", padx=4, pady=3)
    lbl_faturamento_liquido = Label(preco_frame, text="R$ 0.00", bg=COR_CARD, fg=COR_TEXTO, font=("Arial", 11, "bold"))
    lbl_faturamento_liquido.grid(row=1, column=1, sticky="e", padx=4, pady=3)

    Label(preco_frame, text="Total cancelado:", bg=COR_CARD, fg="#b91c1c").grid(row=2, column=0, sticky="w", padx=4, pady=3)
    lbl_total_cancelado = Label(preco_frame, text="R$ 0.00", bg=COR_CARD, fg="#b91c1c", font=("Arial", 11, "bold"))
    lbl_total_cancelado.grid(row=2, column=1, sticky="e", padx=4, pady=3)

    Label(preco_frame, text="Pedidos no período:", bg=COR_CARD, fg=COR_TEXTO).grid(row=3, column=0, sticky="w", padx=4, pady=3)
    lbl_qtd_pedidos = Label(preco_frame, text="0", bg=COR_CARD, fg=COR_TEXTO, font=("Arial", 11, "bold"))
    lbl_qtd_pedidos.grid(row=3, column=1, sticky="e", padx=4, pady=3)

    Label(preco_frame, text="Cancelados:", bg=COR_CARD, fg=COR_TEXTO).grid(row=4, column=0, sticky="w", padx=4, pady=3)
    lbl_qtd_cancelados = Label(preco_frame, text="0", bg=COR_CARD, fg=COR_TEXTO, font=("Arial", 11, "bold"))
    lbl_qtd_cancelados.grid(row=4, column=1, sticky="e", padx=4, pady=3)

    Label(preco_frame, text="Ticket médio (sem cancelados):", bg=COR_CARD, fg=COR_TEXTO).grid(row=5, column=0, sticky="w", padx=4, pady=3)
    lbl_ticket_medio = Label(preco_frame, text="R$ 0.00", bg=COR_CARD, fg=COR_TEXTO, font=("Arial", 11, "bold"))
    lbl_ticket_medio.grid(row=5, column=1, sticky="e", padx=4, pady=3)
...
def preencher_lista_dashboard(frame, titulo_linha, linhas, vazio="Sem dados", use_botoes=False):
    ...
    for texto in linhas:
        if use_botoes:
            Button(
                ...
                wraplength=520,
                cursor="hand2",
                command=lambda t=texto: mensagem_status.config(text=t),
            ).pack(fill="x", pady=4)
        else:
...
def atualizar_dashboard():
    ...
    preencher_lista_dashboard(dash_frame_recentes, "Pedidos recentes", linhas_recentes, use_botoes=True)

    linhas_vendidas = []
    for i, (pizza, qtd) in enumerate(metricas["pizzas_mais_vendidas"], start=1):
        linhas_vendidas.append(f"{i}. {pizza} - {qtd} pedido(s)")
    preencher_lista_dashboard(dash_frame_mais_vendidas, "Mais vendidas", linhas_vendidas, use_botoes=True)
...
frame_menu.pack(side=LEFT, fill="both", expand=True, padx=(0, 8), pady=(0, 4))
frame_menu.configure(height=320)
frame_menu.pack_propagate(False)

frame_form = LabelFrame(top_cards, text="Novo pedido", bg=COR_CARD, fg=COR_MUTED, bd=1, relief="solid", padx=10, pady=8)
frame_form.pack(side=LEFT, fill="both", expand=True, padx=(0, 8), pady=(0, 4))
frame_form.configure(height=320)
frame_form.pack_propagate(False)

frame_clientes = LabelFrame(top_cards, text="Clientes", bg=COR_CARD, fg=COR_MUTED, bd=1, relief="solid", padx=10, pady=8)
frame_clientes.pack(side=LEFT, fill="both", expand=True, pady=(0, 4))
frame_clientes.configure(height=320)
...
frame_lista_clientes_container = Frame(frame_clientes, bg=COR_CARD, height=200)
